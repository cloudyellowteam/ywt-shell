...
RAPD_CMD_FILE=$0
config() {
    local RAPD_PROJECT_NAME=rapd
    local RAPD_PROJECT_NAMESPACE=RAPD
    local RAPD_VERSION="0.0.0-alpha.1"
    ...
    local RAPD_PATH_TMP="${RAPD_CONFIG_PATH_TMP:-"$(dirname -- "$(mktemp -d -u)")"}/${RAPD_PROJECT_NAME}"
    echo '{
        "project": {
            "name": "'$RAPD_PROJECT_NAME'",
            "namespace": "'$RAPD_PROJECT_NAMESPACE'",
            "version": "'$RAPD_VERSION'",
            "path": {
                "packages": "'"$RAPD_PATH_ROOT"'",
                "parent": "'"$RADP_PROJECT_ROOT"'",
                "rapd": "'"$RAPD_PATH_SRC"'",
                "tmp": "'"$RAPD_PATH_TMP"'",
                "bin": "'"$RAPD_PATH_SRC"'"
            }
